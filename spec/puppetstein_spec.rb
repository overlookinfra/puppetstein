...
describe Puppetstein do
  common_opts = ['--platform=centos7-64', '--noop']
  ...
  context 'specifying component versions and using patch mode' do
    ...
    it 'should use most recent puppet-agent#master SHA when PA version is not specified' do
      output = run_puppetstein(common_opts)
      expect(output).to match(/--pre-suite=lib\/setup\/patch\/pre-suite/)
      expect(output).to match(/Using puppet-agent base version latest/)
    end

    ['puppet', 'hiera'].each do |component|
    ...
    it "should trigger a rebuild when a facter version is specified" do
      options = ["--facter=whopper:amazing_test_branch"]
      output = run_puppetstein(common_opts + options)
      expect(output).to match(/--pre-suite=lib\/setup\/build\/pre-suite/)
      expect(output).to match(/Updated.*facter.json with url.* and ref amazing_test_branch/)
    end
  end
  ...
  context 'platform support' do
    ...
    it 'should properly create OSX host configs' do
      options = ['--platform=osx1010-64']
      output = run_puppetstein(common_opts + options)
      expect(output).to match(/Creating host config with targets osx1010-64a/)
    end

    it 'should properly create Windows host configs' do
      options = ['--platform=windows2008r2-64']
      output = run_puppetstein(common_opts + options)
      expect(output).to match(/Creating host config with targets windows2008r2-64a/)
    end
  end
